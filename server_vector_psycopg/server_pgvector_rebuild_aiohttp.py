...
async def handle_insert_impl(req_json, user_id, role_name, conn):
    try:
        ...
        await loop.run_in_executor(None, conn.execute, insert_sql, params)
        # conn.execute(insert_sql, (f"{user_id}-{role_name}", question, embedding))
        # conn.commit()
        # conn.close()
        return 200, {}
    except Exception as e:
...
async def handle_query_impl(req_json, user_id, role_name, conn):
    ...
    try:
        ...
        # 阻塞逻辑
        results = await loop.run_in_executor(None, execute_query, conn, query, params)
        # await 逻辑
        # async with conn.cursor() as cur:
        ...
        if collection_dict.get(role_name) is not None:
            ...
            # result2 = await conn.execute("SELECT knowledge FROM documents WHERE userRole = %(role_name)s",{'role_name': role_name}).fetchall()
            result2 = await loop.run_in_executor(None, execute_query, conn, "SELECT knowledge FROM documents WHERE userRole = %(role_name)s", {'role_name': role_name})
            if result2 and len(result2[0]) > 0:
...
# Remove the unnecessary code that closes the cursor and connection
# cur.close()
# conn.close()
